import { secondsBetween, AlarmState } from '../src/alarmHistory'
import { DescribeAlarmHistoryOutput, AlarmHistoryItem } from 'aws-sdk/clients/cloudwatch';
...
describe('alarmHistory', () => {
  describe('#secondsBetween()', () => {
    it('should calculate the time between two different states', () => {
      ...
      expect(secondsBetween(output, AlarmState.ALARM, AlarmState.OK)).toBe(60)
    })
    ...
    it('should calculate the time between subsequent items of the same state', () => {
      ...
      expect(secondsBetween(output, AlarmState.ALARM, AlarmState.ALARM)).toBe(90)
    })

    it('should fail if history is empty', () => {
      expect(() => secondsBetween({}, AlarmState.ALARM, AlarmState.OK)).toThrow()
    })
    ...
    it('should fail if the requested states are in the wrong order', () => {
      ...
      expect(() => secondsBetween(output, AlarmState.ALARM, AlarmState.OK)).toThrow()
    })
